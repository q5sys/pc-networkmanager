...
void NetworkMan::DevSelectionChanged()
{
   ...
   QString filename = PREFIX + "/share/trueos/xstartup/tray-" + Devs[sel] + ".sh";
...
{    
    ...
    tmp = trueos::Utils::getConfFileValue("/etc/rc.conf", "defaultrouter=", 1);
    if ( tmp.isEmpty() )
    {
       ...
       if ( checkRoute.indexOf("gateway:") != -1 )
        ...
        lineGateway->setText(trueos::Utils::getConfFileValue("/etc/rc.conf", "defaultrouter=", 1) );
    }
    ...
    if( 0 == system("service dhclient status") ){
      ...
      tmp = trueos::Utils::getConfFileValue("/usr/local/etc/dhcpcd.conf", "static domain_name_servers", 1);
    }
    if ( tmp.isEmpty() )
    ...
    lineIPv6DNS1->setText(trueos::Utils::getConfFileValue("/etc/resolv.conf", "nameserver ", "\\b.*:.*:.*\\b", 1) );
    lineIPv6DNS2->setText(trueos::Utils::getConfFileValue("/etc/resolv.conf", "nameserver ", "\\b.*:.*:.*\\b", 2) );
    lineSearchDomain->setText(trueos::Utils::getConfFileValue("/etc/resolv.conf", "search ", 1) );

    // Check for IPv6 settings
    lineIPv6Gateway->setText(trueos::Utils::getConfFileValue("/etc/rc.conf", "ipv6_defaultrouter=", 1) );
    tmp = trueos::Utils::getConfFileValue("/etc/rc.conf", "ipv6_activate_all_interfaces=", 1) ;
    if ( tmp == "YES" )
...
void NetworkMan::refreshDevices()
{
    ...
    while (! Devs[i].isEmpty() )
    {
		...
		if ( DevsType[i] == "Wireless")
		{
		    ...
		    QPixmap PixmapIcon( PREFIX + "/share/trueos/pc-netmanager/pics/network_wifi.png");
		    ...
		    QPixmap PixmapIcon( PREFIX + "/share/trueos/pc-netmanager/pics/network_local.png");
		    listNetDev->addItem(new QListWidgetItem(PixmapIcon.scaled(32,32), Devs[i] + " (" +DevsName[i] + ")") );
		}
...
void NetworkMan::slotSysTray()
{
    ...
    { 
	// Save the new file
        QFile fileout( PREFIX + "/share/trueos/xstartup/tray-" + Devs[sel] + ".sh");
        if ( fileout.open( QIODevice::WriteOnly ) ) {
          QTextStream streamout( &fileout );
          streamout << "sudo pc-nettray " + Devs[sel] + " &";
          fileout.close();
	    
	  runCommand("chmod 755 \"" + PREFIX + "/share/trueos/xstartup/tray-" + Devs[sel] + ".sh\"");
	  runCommand("sync");
	...
	runCommand("rm -f \"" + PREFIX + "/share/trueos/xstartup/tray-" + Devs[sel] + ".sh\"");
    }
   ...
   textGlobalError->setText("");
	...
	QHostAddress addr;
     ...
     if(groupDNS->isChecked()){
       ...
       if ( lineDNS1->text() != "..."  && ! trueos::Utils::validateIPV4(lineDNS1->text() ) ) {
         textGlobalError->setText(tr("Invalid DNS specified"));
	 return;
       }
       if ( lineDNS2->text() != "..."  && ! trueos::Utils::validateIPV4(lineDNS2->text() ) ) {
         textGlobalError->setText(tr("Invalid DNS specified"));
	 return;
       }
     }
     ...
     if ( lineGateway->text() != "..."  && ! trueos::Utils::validateIPV4(lineGateway->text())  && groupGateway->isChecked() ) {
...
void NetworkMan::slotSave()
{
   ...
   if ( !lineDomainName->text().isEmpty() && (lineDomainName->text().toLower() != trueos::Utils::getConfFileValue("/etc/rc.conf", "hostname=lineHostname.lineDomainName", "", 1).toLower() ) ) {
   ...
   if ( lineGateway->text() == "..." || ! groupGateway->isChecked() ) {
     ...
     trueos::Utils::setConfFileValue("/usr/local/etc/dhcpcd.conf","nogateway","",-1);
   } else {
     trueos::Utils::setConfFileValue("/etc/rc.conf", "defaultrouter=", "defaultrouter=\"" + lineGateway->text() + "\"", -1);  
     trueos::Utils::setConfFileValue("/usr/local/etc/dhcpcd.conf","nogateway","nogateway",-1);
   }
   ...
   /* Prefer IPv6 nameservers if IPv6 is enabled. */
    if ( groupIPv6->isChecked() ) {
        if ( ! lineIPv6DNS1->text().isEmpty() ) {
            trueos::Utils::setConfFileValue("/etc/resolv.conf", "nameserver", "nameserver " + lineIPv6DNS1->text(), DNSline);  
            DNSline++;
        }
        if ( ! lineIPv6DNS2->text().isEmpty() ) {
            trueos::Utils::setConfFileValue("/etc/resolv.conf", "nameserver", "nameserver " + lineIPv6DNS2->text(), DNSline);  
            DNSline++;
        }
    }
   
   if ( lineDNS1->text() == "..."  || ! groupDNS->isChecked() ) {
     trueos::Utils::setConfFileValue("/etc/resolv.conf", "nameserver", "", DNSline);
   } else {
     trueos::Utils::setConfFileValue("/etc/resolv.conf", "nameserver", "nameserver " + lineDNS1->text(), DNSline);  
     DNSline++;
   }   
   ...
   if ( lineDNS1->text() != "..." && lineDNS2->text() != "..." && groupDNS->isChecked() ){
     ...
     trueos::Utils::setConfFileValue("/usr/local/etc/dhcpcd.conf", "static domain_name_servers=", "static domain_name_servers=" + lineDNS2->text());  
   }else{
     trueos::Utils::setConfFileValue("/etc/dhclient.conf", "supersede domain-name-servers", "");  
     trueos::Utils::setConfFileValue("/usr/local/etc/dhcpcd.conf", "static domain_name_servers=", "");  
   }

   if ( lineSearchDomain->text().isEmpty() || ! groupDNS->isChecked() ) {
     trueos::Utils::setConfFileValue("/etc/resolv.conf", "search", "", 1);
   } else {
     trueos::Utils::setConfFileValue("/etc/resolv.conf", "search", "search " + lineSearchDomain->text(), 1);
   }

   // Save the IPv6 stuff
    if ( groupIPv6->isChecked() ) {
	trueos::Utils::setConfFileValue("/etc/rc.conf", "ipv6_activate_all_interfaces=", "ipv6_activate_all_interfaces=\"YES\"", -1);
        if ( ! lineIPv6Gateway->text().isEmpty() ) {
	    trueos::Utils::setConfFileValue("/etc/rc.conf", "ipv6_defaultrouter=", "ipv6_defaultrouter=\"" + lineIPv6Gateway->text() + "\"", -1);
        } else {
	    trueos::Utils::setConfFileValue("/etc/rc.conf", "ipv6_defaultrouter=", "", -1);
        }
    } else {
	trueos::Utils::setConfFileValue("/etc/rc.conf", "ipv6_activate_all_interfaces=", "", -1);
	trueos::Utils::setConfFileValue("/etc/rc.conf", "ipv6_defaultrouter=", "", -1);
    }
...
void NetworkMan::loadProxyConfig()
{
   ...
   lineProxyPass->setText(trueos::Utils::getProxyPass());

   tmp = trueos::Utils::getProxyPort();
   tmp.toInt(&ok);
   if ( ok )
      spinProxyPort->setValue(tmp.toInt(&ok));
   

   tmp = trueos::Utils::getProxyType();
   if ( tmp == "digest" )
...
void NetworkMan::saveProxyConfig()
{
   ...
   if ( ! groupProxySettings->isChecked() ) {
         trueos::Utils::setProxyAuth(QString());
         trueos::Utils::setProxyAddress(QString());
   } else {
   	trueos::Utils::setProxyAddress(lineProxyAddress->text() + ":" + tmp.setNum(spinProxyPort->value()));
	...
	if ( ! checkProxyUser->isChecked() || lineProxyUser->text().isEmpty() ) {
	  ...
	  authLine += lineProxyPass->text();
     	  trueos::Utils::setProxyAuth(authLine);
	}
